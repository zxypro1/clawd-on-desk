...
- Hardware Buddy / Clawstick adapter work currently lives on branch `hardware-buddy-state-adapter`. It is env-gated and state-only by default: set `CLAWD_HARDWARE_BUDDY=1` or `CLAWD_HARDWARE_BUDDY_ADDRESS=<device>` to enable it, use `CLAWD_HARDWARE_BUDDY_ROOT` when the sibling `../ClaudeBuddy` checkout is not the bridge-core repo, and do not set `CLAWD_HARDWARE_BUDDY_PERMISSIONS=1` without an explicit secure-link review.
