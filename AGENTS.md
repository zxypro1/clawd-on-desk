...
- Antigravity CLI 通过 `~/.gemini/config/hooks.json` 做 hook 集成；`PreToolUse` 可接 Clawd Allow/Deny 权限气泡；Allow 回包需携带 `decision:"allow"`、兼容字段 `allowTool:true` 和当前 scoped override；Clawd 不写 Antigravity settings 授权规则，也不提供 Antigravity Always Allow；DND / agent 禁用 / 气泡关闭 / Clawd 不可达时必须返回 `ask` 让 agy 原生确认接管
