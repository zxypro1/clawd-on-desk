...
- **Antigravity CLI (agy)** — 通过 `~/.gemini/config/hooks.json` 配置 command hook（已有 Antigravity 配置时 Clawd 启动会自动注册，或执行 `npm run install:antigravity-hooks`）；支持实时状态和实验性 Allow/Deny 权限气泡
...
### 权限审批气泡
- **桌面端权限审批** — 当 Claude Code、Codex CLI、Antigravity CLI、CodeBuddy、opencode 或 Pi 请求受支持的工具权限时，Clawd 会弹出浮动卡片，无需切回终端
